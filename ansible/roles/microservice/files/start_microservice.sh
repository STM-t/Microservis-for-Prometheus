...
##############################################################################
# Скрипт: start_microservice.sh
# Назначение: Автоматическое развертывание Python-микросервиса на сервере
##############################################################################
#
#              Описание:
# Этот скрипт выполняет запуск докер контейнера
# Рекомендуется использовать если докер упал или в том случае, если плейбук не запустил докер
# Сработает только при наличии файлов: Dockerfile, main.py, requirements.txt в директории /opt/microservice
